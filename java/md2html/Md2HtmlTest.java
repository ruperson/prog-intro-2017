...
public class Md2HtmlTest {
    ...
    private void paragraph(final int length, final StringBuilder input, final StringBuilder output, final List<String> markup) {
        int h = checker.randomInt(0, 6);
        String tag = h == 0 ? "p" : "h" + h;
        if (h > 0) {
        ...
        while (input.length() < length) {
            ...
            String middle = checker.randomString(Randomized.ENGLISH);
    ...
    private void randomSpace(final StringBuilder input, final StringBuilder output) {
        if (checker.random.nextBoolean()) {
            String space = checker.random.nextBoolean() ? " " : "\n";
            input.append(space);
            output.append(space);
        }
    }

    private void generate(List<String> markup, StringBuilder input, StringBuilder output) {
        word(input, output);
        if (markup.isEmpty()) {
            return;
        }
        String type = checker.randomItem(markup);
        String tag = TAGS.get(type);
        markup.remove(type);
    ...
    private void word(final StringBuilder input, final StringBuilder output) {
        String prefix = checker.randomString(Randomized.ENGLISH);
        input.append(prefix);
        output.append(prefix);
    }

    private void open(final StringBuilder output, final String tag) {
        output.append("<").append(tag).append(">");
    }

    private void close(final StringBuilder output, final String tag) {
        output.append("</").append(tag).append(">");
    }
    ...
    static class Md2HtmlChecker extends MainFilesChecker {
        ...
        private void test(final String input, final String output) {
            final List<String> run = runFiles(Arrays.asList(input.split("\n")));
            checkEquals(Arrays.asList(output.split("\n")), run);
        }
    }
